    ...
    <div id="taskFormContainer" class="bg-white p-6 rounded-xl border border-emerald-200 shadow-md" style="display: none;">
        ...
        <form action="{{ route('tasks.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4" x-data="{ selectedProject: '', selectedCategory: '' }">
            ...
            <div class="md:col-span-2">
                ...
                <select name="type" onchange="window.location.href='{{ route('dashboard') }}?view=' + this.value" required class="w-full bg-blue-50 border border-blue-200 text-[#0052cc] font-bold rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#0052cc]">
                    <option value="office" {{ $currentType === 'office' ? 'selected' : '' }}>💼 Mode Office</option>
                    <option value="master" {{ $currentType === 'master' ? 'selected' : '' }}>🎓 Mode Master</option>
                </select>
            </div>
            @else
...
{{-- Formulaires de suppression dynamiques pour les projets et catégories filtrés du mode courant --}}
