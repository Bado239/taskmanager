...
<button onclick="modeNormal()" class="bg-gray-700 text-white px-3 py-1 rounded">
☀️ Normal
</button>

<button onclick="modeNuit()" class="bg-black text-white px-3 py-1 rounded border">
🌙 Nuit
</button>

<button onclick="modePapier()" class="bg-yellow-700 text-white px-3 py-1 rounded">
📖 Papier
</button>

<button onclick="pleinEcran()" class="bg-blue-600 text-white px-3 py-1 rounded">
⛶ Plein écran
</button>

<button onclick="changerZoom(-0.1)" class="bg-gray-700 text-white px-3 py-1 rounded">
🔠 A-
</button>
...
<div id="pdfViewer" class="flex-1 overflow-y-auto p-6"></div>
...
<span id="progressText">{{ $book->progress ?? 0 }} %</span>
</div>

<div class="bg-gray-200 h-3 rounded mt-2">
<div id="progressBar" class="bg-blue-600 h-3 rounded" style="width:{{ $book->progress ?? 0 }}%"></div>
</div>

<div class="text-xs text-gray-500 mt-2">
Page :
<span id="currentPage">{{ $book->current_page ?? 1 }}</span>
/
...
<div id="notesZone" class="w-[30%] bg-yellow-50 flex flex-col border-l">
...
name="notes"
id="notes"
class="flex-1 p-5 bg-yellow-50 resize-none">{{ $book->notes }}</textarea>
...
<button
type="submit"
onclick="document.getElementById('statusInput').value='done'"
...
<button
type="submit"
onclick="document.getElementById('statusInput').value='reading'"
...
<h3 class="font-bold text-lg">📖 Définition</h3>

<p id="definitionText" class="mt-3 text-gray-700"></p>

<button onclick="closeDefinition()" class="mt-4 bg-blue-600 text-white px-4 py-2 rounded">
Fermer
</button>
...
// Zoom de rendu du PDF (qualité canvas) — valeur initiale = 100%
let zoom = 1.4;

// Zoom de confort de lecture (transform CSS, boutons A-/A+)
let zoomLecture = 1.4;
...
.then(pdf=>{
    ...
    setTimeout(()=>{
        ...
        let page = document.querySelector(`canvas[data-page="${currentPage}"]`);
...
function loadPage(num){
    ...
    .then(page=>{
        ...
        canvas.className = "mx-auto bg-white shadow";
...
viewer.addEventListener('scroll',()=>{
    ...
    let pages = document.querySelectorAll('#pdfViewer canvas');
    let visiblePage = 1;

    pages.forEach(canvas=>{

        let rect = canvas.getBoundingClientRect();
        let zone = viewer.getBoundingClientRect();

        if(
            rect.top <= zone.top + 250 &&
            rect.bottom >= zone.top + 250
        ){
            visiblePage = parseInt(canvas.dataset.page);
        }
...
function zoomIn(){
    ...
    zoom += 0.2;
    document.getElementById('zoomText').innerHTML = Math.round((zoom/1.4)*100)+"%";
    reloadPages();
...
function toggleNotes(){
    ...
    if(notes.classList.contains('hidden')){
        ...
        pdf.className = "w-[70%] flex flex-col bg-gray-700";
        btn.innerHTML = "📝 Masquer notes";

    } else {

        notes.classList.add('hidden');
        pdf.className = "w-full flex flex-col bg-gray-700";
...
function modeNuit(){
    ...
    document.querySelectorAll('#pdfViewer canvas').forEach(canvas=>{
...
function modePapier(){
    ...
    document.querySelectorAll('#pdfViewer canvas').forEach(canvas=>{
        canvas.style.filter = "sepia(25%) brightness(95%) contrast(90%)";
    });
...
function pleinEcran(){
    ...
    let zone = document.getElementById('pdfZone');
...
// ==========================
// ZOOM LECTURE (confort visuel, transform CSS)
// ==========================
...
function changerZoom(valeur){
    ...
    if(zoomLecture < 0.8){ zoomLecture = 0.8; }
    if(zoomLecture > 2){ zoomLecture = 2; }

    // On applique la transformation au conteneur de page (canvas + textLayer)
    // pour que le calque de texte reste bien aligné avec le rendu du PDF.
    document.querySelectorAll(".page-container").forEach(container=>{
        container.style.transform = "scale("+(zoomLecture/1.4)+")";
    });
...
function showDefinition(word){
    ...
    if(dictionaryCache[word]){
        displayDefinition(word, dictionaryCache[word]);
        return;
    }
    ...
    .then(data=>{
        ...
        displayDefinition(data.word, data.definition);
