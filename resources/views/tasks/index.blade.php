<x-app-layout>
    ...
    <div class="py-6 {{ $themeBg }} min-h-screen font-sans antialiased text-[#1e293b]">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-6 rounded-2xl border border-gray-200/80 shadow-sm gap-4">
                <div>
                    ...
                    <h1 class="text-2xl font-extrabold tracking-tight text-[#0f172a] flex items-center gap-2">
                        {!! $isMaster ? '🎓 Master Academy <span class="text-xs font-semibold px-2.5 py-0.5 bg-purple-50 text-purple-700 rounded-full border border-purple-100">Révisions</span>' : '💼 Office Cockpit <span class="text-xs font-semibold px-2.5 py-0.5 bg-blue-50 text-[#1862ff] rounded-full border border-blue-100">Livrables</span>' !!}
                    </h1>
                </div>
                ...
                <div class="flex bg-gray-100 p-1 rounded-xl w-full md:w-auto border border-gray-200">
                    ...
                    <a href="{{ route('mode.switch', 'master') }}" 
                       class="flex-1 md:flex-none text-center px-5 py-2 rounded-lg text-xs font-bold transition-all {{ $isMaster ? 'bg-white shadow-sm text-purple-600' : 'text-gray-500 hover:text-gray-800' }}">
                        🎓 Master Academy
                    </a>
                </div>
            </div>
                ...
                <div class="lg:col-span-1 bg-white p-6 rounded-2xl border border-gray-200/80 shadow-sm space-y-5">
                    ...
                    @if(!$isMaster)
                        <form action="{{ route('tasks.store') }}" method="POST" class="space-y-4">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Nom du Livrable</label>
                                <input type="text" name="title" required class="w-full text-sm border-gray-200 rounded-xl shadow-sm focus:border-[#1862ff] focus:ring-1 focus:ring-[#1862ff]" placeholder="Ex: Rapport mensuel...">
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Lien de Travail</label>
                                <input type="url" name="document_link" class="w-full text-sm border-gray-200 rounded-xl shadow-sm focus:border-[#1862ff] focus:ring-1 focus:ring-[#1862ff]" placeholder="Drive, Notion, GitHub...">
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Catégorie</label>
                                <select name="category_id" required class="w-full text-sm border-gray-200 rounded-xl shadow-sm focus:border-[#1862ff] focus:ring-1 focus:ring-[#1862ff]">
                            ...
                            <div class="border-t border-gray-100 pt-3 space-y-3">
                                <div>
                                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Associer à un projet</label>
                                    <select name="project_id" id="project_select" class="w-full text-sm border-gray-200 rounded-xl shadow-sm focus:border-[#1862ff] focus:ring-1 focus:ring-[#1862ff]">
                                ...
                                <div id="new_project_field" class="hidden">
                                    ...
                                    <input type="text" name="new_project_name" id="new_project_name" class="w-full text-sm border-blue-200 rounded-xl focus:ring-1 focus:ring-blue-500" placeholder="Nom du projet...">
                                </div>

                                <div>
                                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Étape liée</label>
                                    <select name="project_step_id" id="step_select" class="w-full text-sm border-gray-200 rounded-xl shadow-sm focus:border-[#1862ff] focus:ring-1 focus:ring-[#1862ff]">
                                ...
                                <div id="new_step_field" class="hidden">
                                    ...
                                    <input type="text" name="new_step_name" id="new_step_name" class="w-full text-sm border-blue-200 rounded-xl focus:ring-1 focus:ring-blue-500" placeholder="Nom de l'étape...">
                                </div>
                            </div>

                            <div class="border-t border-gray-100 pt-3 space-y-3">
                                <div>
                                    <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">État du Livrable</label>
                                    <select name="document_status" class="w-full text-sm border-gray-200 rounded-xl shadow-sm focus:border-[#1862ff] focus:ring-1 focus:ring-[#1862ff]">
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Urgence</label>
                                        <select name="priority" class="w-full text-sm border-gray-200 rounded-xl focus:border-[#1862ff]">
                                    ...
                                    <div>
                                        <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Echéance</label>
                                        <input type="date" name="date_prevue" class="w-full text-sm border-gray-200 rounded-xl">
                                    </div>
                                </div>
                            </div>
                    ...
                    @else
                        ...
                        <form action="{{ route('tasks.store') }}" method="POST" class="space-y-4">
                            ...
                            <div>
                                <label class="block text-xs font-bold text-purple-600 uppercase tracking-wider mb-1">Leçon ou Matière</label>
                                <input type="text" name="title" required class="w-full text-sm border-gray-200 rounded-xl focus:ring-1 focus:ring-purple-500" placeholder="Ex: Calcul Stochastique">
                            </div>

                            <div>
                                <label class="block text-xs font-bold text-purple-600 uppercase tracking-wider mb-1">Supports / URL</label>
                                <input type="url" name="document_link" class="w-full text-sm border-gray-200 rounded-xl focus:ring-1 focus:ring-purple-500" placeholder="Lien vers tes cours PDF, TD...">
                            </div>

                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-xs font-bold text-purple-600 uppercase tracking-wider mb-1">Examen</label>
                                    <input type="date" name="date_prevue" class="w-full text-sm border-gray-200 rounded-xl">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-purple-600 uppercase tracking-wider mb-1">Catégorie</label>
                                    <select name="category_id" required class="w-full text-sm border-gray-200 rounded-xl">
                                        @foreach($categories as $category)
                                            <option value="{{ $category->id }}">{{ $category->name }}</option>
                                        @endforeach
                                    </select>
